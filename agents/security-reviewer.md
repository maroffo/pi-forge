...
## Domain Focus: Security Review

Focus on reachable injection, authentication and authorization, tenant or privilege boundaries, secret handling, validation, CSRF and CORS, cryptography, path traversal, supply-chain execution, and fail-open enforcement. Every finding must name the attacker, malicious input, crossed trust boundary, reachable sink, and violated property. Drop claims requiring capabilities the attacker already legitimately possesses.

Use “Security Review” as the report title.
