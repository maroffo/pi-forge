...
The lifecycle extension intercepts ordinary parent `tool_call` events for sensitive paths and Git or shell mutations. Direct Bash-tool commits are always redirected to the source-control gate, which protects `dev`, `main`, and `master`. On an existing non-primary current branch with a clean index and worktree, only a direct exact same-name fully qualified branch push with required config neutralizers for follow-tags, signing, push options, recursive submodules, and force-if-includes, plus a direct `gh pr create` with explicit repository, base, current head, title, and a bounded inline body, consume standing authorization without an interactive prompt. Detached or unborn HEAD, force, tags, deletion, extra refspecs or options, body-file reads, global repository selectors, shell composition, PR modification, and all other remote mutations fail closed into the existing confirmation path. The extension observes successful tool results to invalidate stale verification and queues at most one corrective follow-up per observed source-mutation sequence. Protected implementation writers run as a single direct foreground call, without supervisor or nested-subagent tools, so their result is joined before this final verification boundary.
