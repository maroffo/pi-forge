...
- Every other Git push or PR mutation, destructive Git state change, recursive deletion, privilege escalation, and broad permission change requires one interactive confirmation for the exact tool call. Headless execution fails closed.
...
This extension is a workflow control, not a shell sandbox or complete command parser. The narrow standing-authorized push and PR-create forms are syntactic allowlists, not proof of repository ownership, remote policy, CI success, or semantic payload safety. Obfuscated shell, aliases, custom tools, direct filesystem APIs inside a process, and project code can bypass tool-name and command-shape observation. Bash commands can mutate source without a write/edit event. Git aliases can hide subcommands. Interactive confirmation establishes intent for one visible call, not containment.
