...
Invoking this skill authorizes clarification in the current conversation, one protected analyst call after a self-contained artifact is ready, and one local eligibility call to `convene_opt_in_expert_panel` after a complete recommendation. It does not itself authorize source edits, project-wide discovery, another provider, or Expert Panel disclosure.

Use the current parent provider/model for the analyst. A different provider requires separate disclosure and explicit approval before launch. Never treat the initial skill invocation, an analyst recommendation, silence, or an ambiguous reply as approval for manual Second Opinion. Automatic provider launch is authorized only by a separate unused standing grant created interactively with `/auto-panel enable`; the extension remains the authority for that state.
...
4. if the tool launches or reports an unknown outcome, stop and never retry;
5. if it reports disabled, consumed, headless, or payload-policy rejection, explain that no automatic provider call occurred and ask one separate yes-or-no question about using the manual reviewed Second Opinion path.

The automatic tool, not the model, decides whether an unused standing grant exists. Never echo content rejected by its payload policy. Never call the automatic tool from a panel result, synthesis, resumed child, or any workflow other than the direct complete Socratic recommendation; its one-shot state is defense in depth against recursion.

Only an unambiguous yes after the manual fallback question authorizes preparation through the `second-opinion` skill. A no, cancellation, silence, unrelated response, or earlier approval launches nothing manually. After explicit yes, load and follow `second-opinion`, then call `convene_expert_panel` exactly once. Its editable payload and digest-bound multi-provider consent remain mandatory. A child or skill never invokes `/second-opinion` or `/expert-panel` directly.
